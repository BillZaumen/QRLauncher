...
public class Installer {
    ...
    private static boolean CLEANUP_QRLDIR_ON_ERROR = false;
    ...
    private static Path qrldir = null;
	...
	if (notDryrun && old != null) {
		...
		if (Files.isDirectory(qrldir)) {
		    try {
			clearDirectory(qrldir);
			Files.delete(qrldir);
			Files.move(old, qrldir);
	...
	if (windowsOS) {
		...
		JOptionPane.showMessageDialog(null,
					      ...
					      + "missing",
					      "QRLauncher Installer Error",
					      JOptionPane.ERROR_MESSAGE);
		System.exit(1);
	    } else {
		wdirname = "%PROGRAMDATA%"
		    + fs.getSeparator() + "QRLlauncher";
	    }
	    ...
	    if (Files.isDirectory(fs.getPath("/usr/local/share/bzdev"))) {
		ulButton.setSelected(true);
		Path p =
		    fs.getPath("/usr/local/share/bzdev/man/man1/lsnof.1.gz");
		if (Files.exists(p)) {
		    gzipManCheckBox.setSelected(true);
		}
	    } else if (Files.isDirectory(fs.getPath("/opt/bzdev"))) {
		...
		Path p =
		    fs.getPath("/opt/bzdev/man/man1/lsnof.1.gz");
		if (Files.exists(p)) {
		    gzipManCheckBox.setSelected(true);
		}
	    } else {
	...
	runButton.addActionListener(new AbstractAction() {
		public void actionPerformed(ActionEvent event) {
			...
			if (!isWindows && ulButton.isSelected()) {
				...
				(fs.getPath("/usr/local/share/bzdev"))) {
			...
			if (!isWindows
			    ...
			    if (!Files.isDirectory(fs.getPath("/opt/bzdev"))) {
				JOptionPane.showMessageDialog
				    (frame,
			...
			JOptionPane.showMessageDialog
			    (frame, tf, "QRLauncher Installer Java Command",
			...
			JOptionPane.showMessageDialog
			    (frame, "Installer will exit - no errors detected",
			     "QRLauncher Installer",
			     JOptionPane.INFORMATION_MESSAGE);
			System.exit(0);
		    } catch(Exception e) {
			cleanupOnError();
			JOptionPane.showMessageDialog
			    (frame, e.getMessage(), "QRLauncher Installer Error",
			     JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		    } catch(Error e) {
			cleanupOnError();
	...
	String qrldirString = null;	 // use only for Windows.
	...
	if (usrlocalMode) {
	    qrldir = installPath.resolve("share");
	    int lcnt = 0;
	    while (Files.isSymbolicLink(qrldir)) {
		qrldir = qrldir.resolveSibling
		    (Files.readSymbolicLink(qrldir));
	    }
	    qrldir = qrldir.resolve("QRLauncher");
	} else {
	    qrldir = installPath.resolve("QRLauncher");
	}
	if (installPathString != null) {
	    qrldirString = installPathString
		+ System.getProperty("file.separator") + "QRLauncher";
	}
	Path ourOld =  qrldir.resolveSibling("QRLauncher-old");
	tspath = qrldir.resolve(tsname);

	cvrbin = qrldir.resolve("bin");
	cvrdoc = qrldir.resolve("doc");
	if (posixMode) {
	    cvrman = qrldir.resolve("man");
	    cvrman1 = cvrman.resolve("man1");
	}
	// Handle uninstall ; otherwise save current installation in case
	// of errors.
	if (uninstall || (Files.isDirectory(qrldir) && Files.exists(tspath))) {
		...
		if (Files.exists(qrldir)) {
		    throw new Throwable("File " + qrldir
				   + " exists and is not a directory"
				   + " or cannot be deleted");
		}
	    } else {
		try {
		    if (notDryrun) {
			Files.move(qrldir, ourOld);
		    } else {
			System.out.println("would move " + qrldir
					   + " to " + ourOld);
		    }
		    old = ourOld;
		} catch (Exception e) {
		    throw new Throwable("could not move " + qrldir
					+ " to " + ourOld
					+ "(" + e.getClass().getName()
					+ ": "
					+ e.getMessage() + ")");
	...
	if (!Files.exists(qrldir)) {
	    createDir(qrldir, dirPerm);
	} else if (!Files.isDirectory(qrldir)) {
	    System.out.println("Expected a directory: " + qrldir);
	    System.exit(1);
	}
	clearDirectory(qrldir);
	...
	for (String name: docfiles) {
	    ...
	    file = file.replace("BZDEVDIR", qrldir.toString());
	    if (!posixMode) {
		file = file.replace("\n", System.getProperty("line.separator"));
	    }
	    byte[] barray = file.getBytes("UTF-8");
	...
	for (String name: libraries) {
		...
		createFile(qrldir, fs.getPath(name), dirPerm, filePerm, is);
		is.close();
	}
